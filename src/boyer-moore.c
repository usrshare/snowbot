...
uint32_t boyer_moore (const uint8_t *string, uint32_t stringlen, const uint8_t *pat, uint32_t patlen) {
    int i;
...
void test(uint8_t *string, uint8_t *pat) {
    ...
    uint32_t pos = boyer_moore(string, strlen((const char*)string), pat, strlen((const char*)pat));
#ifdef DEBUG
    printf("\n");
#endif
    if (pos == 0 && chars_compared != strlen((const char*)pat))
        printf("Not Found - ");
    else
